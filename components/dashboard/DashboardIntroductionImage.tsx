...
const DashboardIntroductionImage = ({ isMobile }: Props) => {
  ...
  const renderImage = (name: string, w: number, h: number, classTxt?: string) => {
    const staticImageURL = isPC ? staticImageURLPC : isTablet ? staticImageURLTablet : staticImageURLMobile
    return (
      <div
        className={`homeImageBoxShadow ${isPC ? "rounded-[20px]" : isTablet ? "rounded-[13px]" : "rounded-[8px]"} ${classTxt}`}
      ...
      >
        <Image
          ...
          width={w} height={h}
          quality={80}
          className={`${isPC ? "rounded-[20px]" : isTablet ? "rounded-[13px]" : "rounded-[8px]"}`}
          priority
        />
      </div>
    )
    ...
    return (
      <>
        <div
          className='w-screen'
          style={{
            backgroundImage: `url(${staticImageURLPC + '/home_main.png'})`,
            width: '100vw',
            height: 'calc(100vw / 1538 * 1048)',
        ...
        {/* {renderImage('/home_payment.webp', 238, 386, 'mb-[24px]')} */}
        ...
        {/* {renderImage('/home_contact_host.webp', 236, 185, 'mb-[24px]')} */}
        {/* <div className="w-full h-6" /> */}
        {/* {renderImage('/home_map.webp', 236, 278, 'mb-[24px]')} */}
        {/* <div className="w-full h-6" /> */}
  ...
  const renderTablet = () => {
    return (
      <>
        ...
        <div className="flex flex-col ml-[13px] min-w-[553px] pr-[16px]">
  ...
  return (
    <div
      className={`flex lg:justify-center w-[calc(100%_+_32px)] mx-[-16px] overflow-auto hidden-scroll-bar`}
      style={{
        minHeight: isPC ? 'auto' : isTablet ? 450 : 280,
        marginTop: isPC ? 160 : isTablet ? 112 : 64,
        paddingLeft: isPC ? 0 : isTablet ? 16 : 16,
        paddingRight: 0,
      }}
      id="home_img_group"
    >
      {isPC ? renderPC() : isTablet ? renderTablet() : renderMobile()}
    </div>
  );
};

export default DashboardIntroductionImage;