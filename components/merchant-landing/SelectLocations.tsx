...
import { isEmpty, showLocation } from '@dtravel/utils/common'
...
const SelectLocations: React.FC<Props> = ({ hostID, transformOrigin, anchorOrigin, anchorElement }) => {
  ...
  const handleSelectLocation = (city: string, state: string, country: string) => {
    const cityAndCountry = showLocation(city, state, country)
  ...
  const renderContent = () => {
    return (
      <div className={''}>
        ...
        {/*---Body---*/}
        <div className={'max-h-[414px] overflow-y-auto md:px-[24px] hidden-scroll-bar'}>
          {Array.isArray(locations) &&
            locations.map((item, index) => {
              const isLastItem = index === locations.length - 1
              if (isEmpty(showLocation(item.city, item.state, item.country))) return <></>
              return (
                <button
                  key={`location_${index}`}
                  className={`w-full flex justify-between items-center h-[64px] border-[#E5E5E0] ${isLastItem ? '' : 'border-b'
                    }`}
                  onClick={() => handleSelectLocation(item.city, item.state, item.country)}
                >
                  <span className={'font-inter-500 text-14-18 text-neutral-900 flex h-[24px] items-center'}>
                    {showLocation(item.city, item.state, item.country)}
                  </span>
                  {selectedLocations.includes(showLocation(item.city, item.state, item.country)) && (
