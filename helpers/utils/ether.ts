...
import { setAccounts, setChainId, setToastError } from '@dtravel/redux/slices/common'
...
export const handleBookCrypto = async (
  ...
  const isNeedApproved = tokenAddress !== '0x0000000000000000000000000000000000000000'
  try {
    const {
      token,
      bookingId,
    ...
    const bookingParameters = {
      ...
      checkIn: checkinDateInTimestamp,
      checkOut: checkoutDateInTimestamp,
      expireAt: bookingExpirationInTimestamp,
      bookingAmount: utils.parseUnits(String(bookingAmount), decimal),
      guest: guestWallet,
      property: propertyContract,
      referrer: ethers.constants.AddressZero,
      policies: cancellationPolicies.map((p: any) => {
        return {
          expireAt: p.expiryTime,
          refundAmount: utils.parseUnits(String(p.refundAmount), decimal),
        }
      }),
      insuranceInfo: {
        damageProtectionFee: utils.parseUnits(String(damageProtectionFee), decimal),
        feeReceiver: feeReceiver || ethers.constants.AddressZero,
        kygStatus: Number(kygStatus || 0),
      },
    }
    const provider = new ethers.providers.Web3Provider((window as any)?.ethereum)
    let baseNonce = provider.getTransactionCount(guestWallet)
    let nonceOffset = 0
    const getNonce = () => {
    ...
    // const allowance = await contract.allowance(guestWallet, propertyContract)

    // if (!bookingParameters.bookingAmount.lte(allowance)) {
    // try {
    //   const res = await contract.callStatic.approve(propertyContract, bookingParameters.bookingAmount, {
    //     nonce: getNonce(),
    //   })
    //   console.log('success callstatic book', res)
    // } catch (error) {
    //   console.log('error callstatic book', error)
    // }
    if (isNeedApproved) {
      // book with bsc network
      await contract.approve(propertyContract, bookingParameters.bookingAmount, {
        nonce: getNonce(),
      })
      contract.on('Approval', async (from, to, amount) => {
        // console.log('approval event from', from, guestWallet);
        // console.log('approval event to', to, propertyContract);
        // console.log('approval event amount', amount);
        const isSuccess =
          from.toLowerCase() === guestWallet.toLowerCase() &&
          String(to).toLowerCase() === propertyContract.toLowerCase()
        // console.log('isSuccess', isSuccess);
        if (isSuccess) {
          contract.removeAllListeners('Approval')
          // handle book transfer
          const newContract = new ethers.Contract(propertyContract, PROPERTY_CONTRACT_ABI, signer)
          // try {
          //   const res = await newContract.callStatic.book(bookingParameters, signature, { nonce: getNonce() })
          //   console.log('success callstatic book', res)
          // } catch (error) {
          //   console.log('error callstatic book', error)
          // }
          const book = await newContract.book(bookingParameters, signature, { nonce: getNonce() }).catch((err: any) => {
            setLoading(false)
            store.dispatch(setToastError({ message: err?.data?.message || err?.message }))
          })
          setLoading(false)
          if (book) {
      ...
      // book with ethereum
      const newContract = new ethers.Contract(propertyContract, PROPERTY_CONTRACT_ABI, signer)
      const book = await newContract
        .book(bookingParameters, signature, { nonce: getNonce(), value: bookingParameters.bookingAmount })
        .catch((err: any) => {
          setLoading(false)
          store.dispatch(setToastError({ message: err?.data?.message || err?.message }))
        })
    ...
    setLoading(false)
    store.dispatch(setToastError({ message: error?.data?.message || error?.message }))
  }
}
...
export const cancelBookingSmartContract = async (bookingId: string, guestWallet: string, propertyContract: string) => {
  const provider = new ethers.providers.Web3Provider((window as any)?.ethereum)
  const signer = provider.getSigner(guestWallet)
